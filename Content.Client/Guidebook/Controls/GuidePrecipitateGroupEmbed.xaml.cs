...
public sealed partial class GuidePrecipitateGroupEmbed : BoxContainer, IDocumentTag
{
    ...
    public GuidePrecipitateGroupEmbed()
    {
    ...
    public GuidePrecipitateGroupEmbed(string group) : this()
    {
        ...
        {
            var embed = new GuidePrecipitateEmbed(reagent);
    ...
    public bool TryParseTag(Dictionary<string, string> args, [NotNullWhen(true)] out Control? control)
    {
        ...
        Dictionary<string, GuidePrecipitateEmbed> precipitateDict = new Dictionary<string, GuidePrecipitateEmbed>();
        var prototypes = _prototype.EnumeratePrototypes<ReactionPrototype>()
            .Where(p => !p.Source && p.Effects.Length >= 1 && p.Group.Equals(group))
            .OrderBy(p => p.Priority)
            .ThenBy(p => p.Products.Count)
            .ToList();
        foreach (var reagent in prototypes)
        {
            ...
            if (precipitateDict.ContainsKey(spawnId))
                continue;
            precipitateDict.Add(spawnId, new GuidePrecipitateEmbed(reagent));
        }
